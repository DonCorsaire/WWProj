...
public class DialogueGenerator : MonoBehaviour {
	...
	public void Generate()
	{
		...
		for(int j = 0; j < node.Length; j++)
		{
			...
			userNode.Attributes.Append(attribute);
			attribute = xmlDoc.CreateAttribute("nextNode");
			attribute.Value = node[j].nextNode.ToString();
			userNode.Attributes.Append(attribute);
			...
			for (int i = 0; i < node[j].playerAnswer.Length; i++)
			{
				...
				if (node[j].playerAnswer[i].dialogueInteract != DialogueInteract.None) element.SetAttribute("dialogueInteract", node[j].playerAnswer[i].dialogueInteract.ToString());
